with open('Protein/For AAI/PneumoVSPyo_prot_sorted.txt', 'r') as agaFile:
...
def taking_lenght_BBH(allLines):
    toSend = 0
    for line in allLines:
        if line.split("\t")[0] == "CDS":
            toSend += int(line.split("\t")[17])
    return toSend

sum_of_Pyo = taking_lenght_BBH(pyoFTLines)
    ...
    for line in allLines:
        ...
        if dispatchElement[0] == "CDS":
            if dispatchElement[10] == "":
                toSend_spy[dispatchElement[16]] = dispatchElement[18]
            else:
                toSend_wp[dispatchElement[10]] = dispatchElement[18]
...
final_result = sum(value for value in list_of_lenght)/sum_of_Pyo
print(final_result)